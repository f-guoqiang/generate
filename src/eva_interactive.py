...
def main():
    ...
    #
    # @app.route("/service/api/xinaliao", methods=["GET", "POST"])
    # def medical_ner():
    #     # 定义输出格式为字典，一个key是success,一个键是下面的data键
    #     data = {"sucess": 0}
    #     result = []
    ...
    #
    #     # 逻辑，crf只分两类，is-robot和其他（其他），每次都过crf
    #     # 这样就可以和闲聊保持上下文沟通，使用clear进行换话题
    #     result = generate_samples(model, tokenizer, args, torch.cuda.current_device())
    #     print(text_list)
    #     print(result)
    #
    #     data["data"] = result
    ...
    #
    # server = pywsgi.WSGIServer(("0.0.0.0", 60080), app)
